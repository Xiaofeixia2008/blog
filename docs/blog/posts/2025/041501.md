...
```mcfunction {linenums="1"}
execute if blocks -1 22 85 -1 22 85 10 5 7 all unless entity @e[type=item] if entity @e[type=skeleton] unless entity @a[hasitem={item=skeleton_spawn_egg}] if score adv1 advancement matches 0 if score adv0 advancement matches 1 run say 114514
```
...
以下代码为判定游戏失败部分，循环执行，此处使用`say`命令表示游戏失败。
...
以下代码在小游戏进行过程中循环执行，此处使用`say`命令表示已完成划船。
