...
```mcfunction {linenums="1"}
execute if blocks -1 22 85 -1 22 85 10 5 7 all unless entity @e[type=item] if entity @e[type=skeleton] unless entity @a[hasitem={item=skeleton_spawn_egg}] if score adv1 advancement matches 0 if score adv0 advancement matches 1 run say 114514 # (2)!
```

2. 按原题目要求，实际上最后一部分应该写成`run say 恭喜你获取进度！`
